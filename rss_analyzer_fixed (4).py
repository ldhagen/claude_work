...
class RSSWordAnalyzer:
    ...
    def analyze_feeds(self):
        ...
        return df, word_freq_df, feed_word_counts

def create_html_template():
    """Create the HTML template content"""
        ...
        function renderFeeds(selectedFeeds, defaultFeeds) {
            ...
            for (const [name, url] of Object.entries(defaultFeeds)) {
                ...
                feedItem.innerHTML = '<label><input type="checkbox" ' + 
                    ...
                    ' onchange="toggleFeed(\\'' + name + '\\', \\'' + url + '\\', this.checked)"><div><strong>' + 
        ...
        function displayFeedBreakdown(feedWordCounts) {
            ...
            feedNames.forEach((feedName, index) => {
                ...
                if (words && words.length > 0) {
                    ...
                    table.innerHTML = '<thead><tr><th>Rank</th><th>Word</th><th>Frequency</th></tr></thead>';
...
def analyze():
    ...
    try:
        articles_df, word_freq_df, feed_word_counts = analyzer.analyze_feeds()
